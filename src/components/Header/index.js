...
const Header = (props) => {
  ...
  const _renderContent = useCallback(
    () => {
      const profileItems = getLocalStorage('profile');
      const latestProfile = (profileItems && profileItems.length > 0)
        ? profileItems[profileItems.length - 1]
        : null;
      if(latestProfile){
        return (
            ...
            <button onClick={() => setSate(!show)} className="bg-gray-800 flex text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" id="user-menu" >
              ...
              <div className="h-8 w-8 rounded-full overflow-hidden">
                <img
                  ...
                  src={latestProfile.userData && latestProfile.userData.picture}
                  alt="user"
                  referrerPolicy="no-referrer"
                  onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = blankUserImage; }}
                />
              </div>
            </button>
            <Transition
          ...
          <div className="ml-3 relative">
            <div>
              <button onClick={() => setSate(!show)} className="bg-gray-800 flex text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" id="user-menu" >
                <span className="sr-only">Open user menu</span>
                <div className="h-8 w-8 rounded-full overflow-hidden">
                  <img className="h-full w-full object-cover object-center block" src={blankUserImage} alt="user-blank" referrerPolicy="no-referrer" />
                </div>
              </button>
            </div>
          </div>
